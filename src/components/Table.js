    ...
    return (
        <table className="table table-striped table-sm" data={currentUSValues}>
            ...
            <tbody>
                ...
                <tr><td>Hospitalizations</td><td>{currentUSValues.hospitalized && currentUSValues.hospitalized.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td><td><FontAwesomeIcon className={currentUSValues.hospitalizedIncrease > 0 ? "increase" : "decrease"} icon={currentUSValues.hospitalized > 0 ? faCaretUp : faCaretDown} aria-label="decrease or increase hospitalizations" /> {currentUSValues.hospitalizedIncrease && currentUSValues.hospitalizedIncrease.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td></tr>
